...
router.get('/me/profile', protect, requireRole('tailor'), async (req, res, next) => {
  try {
...
router.get('/me/reviews', protect, requireRole('tailor'), async (req, res, next) => {
  try {
    const tailor = await Tailor.findOne({ owner: req.user._id })
    if (!tailor) return res.status(404).json({ message: 'Tailor profile not found' })
...
router.get('/customer/my-reviews', protect, requireRole('customer'), async (req, res, next) => {
  try {
    const { page = 1, limit = 10 } = req.query
    const [reviews, total] = await Promise.all([
...
router.post('/', protect, requireRole('tailor'), async (req, res, next) => {
  try {
    ...
    // First 100 tailors get 6 months free early-bird trial
    const totalCount = await Tailor.countDocuments()
    const isEarlyBird = totalCount < 100
...
router.patch('/me', protect, requireRole('tailor'), async (req, res, next) => {
  try {
    const forbidden = ['owner', 'isVerified', 'isTopRated', 'status', 'isActive', 'subscriptionType', 'subscriptionExpiry', 'isEarlyBird', 'freeTrialEnds', 'trialReminderSent', 'rating', 'reviewCount']
...
router.post('/reviews/:reviewId/reply', protect, requireRole('tailor'), async (req, res, next) => {
  try {
    const tailor = await Tailor.findOne({ owner: req.user._id })
    if (!tailor) return res.status(404).json({ message: 'Tailor profile not found' })
