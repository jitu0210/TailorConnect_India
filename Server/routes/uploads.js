...
router.post('/profile', protect, uploadProfileImage.single('image'), async (req, res) => {
...
router.post('/gallery', protect, requireRole('tailor'), uploadGallery.single('image'), async (req, res) => {
  if (!req.file) return res.status(400).json({ message: 'No file uploaded' })
  const tailor = await Tailor.findOne({ owner: req.user._id })
  if (!tailor) return res.status(404).json({ message: 'Tailor profile not found' })
...
router.delete('/gallery/:itemId', protect, requireRole('tailor'), async (req, res) => {
  const tailor = await Tailor.findOne({ owner: req.user._id })
  if (!tailor) return res.status(404).json({ message: 'Tailor profile not found' })
  ...
  if (item.publicId) {
    await cloudinary.uploader.destroy(`tailorconnect/tailors/gallery/${item.publicId}`).catch(() => {})
  }
...
router.post('/review-photos', protect, uploadReviewPhotos.array('photos', 3), async (req, res) => {
